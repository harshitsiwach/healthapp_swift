...
actor HealthManager {
    ...
    func fetchLatestHeartRate() async -> Int? {
        ...
        return await fetchLatestQuantity(for: type)
    }
    ...
    private func fetchLatestQuantity<T>(for type: HKQuantityType) async -> T? where T: LosslessStringConvertible {
        ...
        return await withCheckedContinuation { continuation in
            let query = HKSampleQuery(sampleType: type, predicate: nil, limit: 1, sortDescriptors: [sort]) { _, samples, error in
                ...
                continuation.resume(returning: T("\(value)") as? T)
            }
            store.execute(query)
        }
    }
